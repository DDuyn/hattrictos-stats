...
function StandingsGroup(props: {
  ...
  function rowAccentStyle(position: number): string {
    ...
    if (
      props.relegationSlots > 0 &&
      position > maxPosition() - props.relegationSlots
    )
      return "border-left: 3px solid #f87171"; // red-400
    return "border-left: 3px solid transparent";
  }

  function formDotStyle(result: FormResult): string {
    if (result === 'W') return 'background:#34d399';  // green
    if (result === 'D') return 'background:#d1d5db';  // gray
    return 'background:#f87171';                      // red
  }
          ...
          <tbody>
            <For each={props.rows}>
              {(row) => {
                ...
                const form = () => getTeamRecentForm(props.matches, row.htTeamId);
                return (
                  <tr
                    class="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors"
                    style={rowAccentStyle(row.position)}
                  >
...
function MatchRow(props: {
  ...
  const inner = () => (
    <div class="flex flex-col py-2.5 border-b border-gray-50 last:border-0 gap-0.5">
      ...
      <div class="flex items-center gap-2 min-w-0">
        <span
          class={`text-sm flex-1 text-right leading-tight ${finished && m.homeGoals! > m.awayGoals! ? "font-semibold text-gray-900" : "text-gray-600"}`}
        >
        ...
        <span
          class={`text-sm font-mono shrink-0 w-14 text-center rounded px-1.5 py-0.5 ${finished ? "bg-gray-100 text-gray-900 font-semibold" : "text-gray-300"}`}
        >
          {finished ? `${m.homeGoals} - ${m.awayGoals}` : "vs"}
        </span>
        <span
          class={`text-sm flex-1 leading-tight ${finished && m.awayGoals! > m.homeGoals! ? "font-semibold text-gray-900" : "text-gray-600"}`}
        >
  ...
  return (
    <Show when={hasDetail} fallback={inner()}>
      <A
      ...
      <Show when={ctrl.state.detail !== null}>
        {(_) => {
          ...
          const activeRoundMatches = createMemo(() =>
            ctrl.activeRound() !== null
              ? (matchRounds.get(ctrl.activeRound()!) ?? [])
              : [],
          );
          ...
          return (
            <>
                ...
                <div class="flex items-center justify-between gap-4 flex-wrap">
                  <div class="flex items-center gap-3 min-w-0">
                    ...
                    <Show when={t.season !== null}>
                      <span class="text-sm text-gray-400 bg-gray-100 rounded px-2 py-0.5 shrink-0">
                        Temporada {t.season}
                      </span>
                    </Show>
                  </div>
                  <A
                    href={`/torneos/${t.id}/estadisticas`}
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-200 text-sm text-gray-600 hover:border-gray-400 hover:text-gray-900 transition-colors shrink-0"
                  >
                    <svg
                      class="w-4 h-4"
                      fill="none"
              ...
              <div class="flex flex-col gap-8">
                ...
                <Show
                  when={nextRound() !== null && nextRoundMatches().length > 0}
                >
                  <section>
                    <h2 class="text-base font-semibold text-gray-900 mb-3">
                      Próxima jornada
                  ...
                  <section>
                    ...
                    <div class="flex items-center gap-2 mb-4">
                      <button
                        ...
                        onClick={() => {
                          const idx = roundNumbers().indexOf(
                            ctrl.activeRound()!,
                          );
                          if (idx > 0)
                            ctrl.setActiveRound(roundNumbers()[idx - 1]);
                        }}
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          ctrl.setActiveRound(Number(e.currentTarget.value))
                        }
